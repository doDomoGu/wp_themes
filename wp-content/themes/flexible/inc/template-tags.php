...
function flexible_posted_on() {
	...
	$posted_on = sprintf(
		esc_html_x( '%s', 'post date', 'flexible' ),
		'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
	);

	$byline = sprintf(
		esc_html_x( '%s', 'post author', 'flexible' ),
	...
	if ( ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
		...
		comments_popup_link( esc_html__( 'Comment (0)', 'flexible' ), esc_html__( 'Comment (1)', 'flexible' ), esc_html__( 'Comments (%)', 'flexible' ) );
		echo '</span>';
	}
